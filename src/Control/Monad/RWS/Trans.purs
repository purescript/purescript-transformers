...
evalRWST m r s = runRWST m r s >>= \(RWSResult _ result writer) -> pure (Tuple result writer)
...
execRWST m r s = runRWST m r s >>= \(RWSResult state _ writer) -> pure (Tuple state writer)
